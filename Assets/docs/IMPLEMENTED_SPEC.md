...
| `ApplyResearchBoost(multiplier, duration)` | グローバル研究速度バフを付与、SaveDataに永続化(ひらめきのしずく用) |
...
GetResearchSpeed = BaseResearchSpeed(PetMaster, 固定1.0)
                  ...
                  × ResearchBoostMultiplier(グローバルバフが有効な場合のみ、通常1.0)
                  × CheerMultiplier(このキャラの応援バフが有効な場合のみ、通常1.0)
```
...
**おかえりまとめ(2026-07-20追加)**: `WordsLearnedOffline`/`LevelUpsOffline`プロパティを追加。`CalculateOfflineProgress()`が`ResearchSystem.UpdateResearch()`を呼ぶ前後だけ`OnWordUnlocked`/`OnPetLevelUp`を一時購読してカウントし(EventBusへの純粋なオブザーバーとして動作するだけで、他のシステムの挙動は変えない)、`HomeUIRoot`の放置報酬Toastが「◯時間◯分放置しました！／言霊 +◯／新しいことば +◯／レベルアップ +◯」と複数行でまとめて表示する(該当項目が0件の行は表示しない)。単なる金額表示だった放置報酬を、離れている間に何が起きたかが一目でわかる形に強化した。
...
```
Canvas
├── World (HomeWorldView) - もじの庭。Screen.safeArea基準でヘッダー/フッターの間に収まるビューポート+スクロール可能な庭(5.5節/6.5節参照)
├── Header (Screen.safeArea上端基準、高さUiTheme.HeaderHeight、raycastTarget無効)
│   ├── 言霊表示テキスト(上半分)
│   └── 目標インジケーター(下半分、2026-07-20追加。下記参照)
├── Footer (Screen.safeArea下端基準、高さUiTheme.FooterHeight、raycastTarget無効)
│   └── ButtonRow (HorizontalLayoutGroupで自動整列)
...
起動時に`OnMoneyChanged`/`OnResearchCompleted`/`OnPetLevelUp`/`OnPetUnlocked`を購読し、それぞれ言霊表示更新・「新しいことば！」Toast・「レベルアップ！」Toast・手書きモーダル起動(6.5節参照)を行う。起動直後、直近の放置時間が0超なら放置報酬Toastも表示する。

**目標インジケーター(2026-07-20追加)**: 研究が完全自動化され受動的なループになったため、「今どこに向かっているか」を示す一言を常時表示する。図鑑の解放数を50語区切りのマイルストーンで丸め、「目標: 図鑑{次の節目}語まであと{残り}語」と表示(全語収集済みなら「目標: 図鑑コンプリート達成！」)。`OnResearchCompleted`で単語が解放されるたびに再計算する軽量な表示専用機能で、新しいセーブデータや永続状態は持たない。
...
- `PetToken`: 1体の文字を表すTextMeshPro(手書き済みなら`HandwritingSystem`の保存画像を代わりに表示)。3〜6秒のランダム待機後、庭(スクロール領域のContent、画面そのものではない)内のランダム位置へ移動する処理をUniTaskループで繰り返す(**MonoBehaviour.Updateは不使用**)。
- **タップ操作の使い分け(2026-07-20刷新)**: `Button`コンポーネントは廃止し、`IPointerDownHandler`/`IPointerClickHandler`を直接実装。`OnPointerClick`はUnityが「ドラッグ閾値を超えなかった押下+離しのみ」で発火することを保証するため、庭をスワイプでスクロールする操作と衝突しない。押下から離すまでが0.45秒以上なら長押し扱いで詳細画面(`OpenPetDetail`)を開く。それ未満の素早いタップは「なで」の1回としてカウントされ、1.2秒以内に3回連続でタップすると`PetSystem.Stroke()`を呼び「💗」の反応(下記参照)を表示する。以前あったメニューの「なでる」ボタンは撤去し、庭で直接触れる操作に一本化した(`PetDetailView`からは削除済み)。
- **反応ポップアップ**(2026-07-20追加): `PetToken.ShowReactionAsync(emoji)`が、絵文字1つを頭上にふわっと出して0.8秒でフェードアウトさせる汎用ヘルパー。なで(💗)・レベルアップ(✨)・単語完成(✨)の3箇所で共用。
- **レベルアップ演出**(2026-07-20追加): `OnPetLevelUp`受信時、見た目成長の更新に加えて「✨」反応+0.15秒で1.3倍に拡大→0.2秒で元のサイズに戻る、というバウンス演出を再生する(`AnimateScaleAsync`)。
- **性格(徘徊速度のばらつき)**(2026-07-20追加): `characterId`から生成した専用の乱数(位置決定用の乱数とは別シード)で0.7〜1.3倍の速度係数を算出し、待機時間・移動時間の両方に適用。同じ文字は毎回同じ「性格」になる(セーブ跨ぎでも再現される、状態として保存はしていない決定論的な値)。
- **文字が集まって単語になる**(2026-07-20追加): `HomeWorldView`が`OnResearchCompleted`を購読し、完成した単語を構成する文字(`WordSystem.GetCharacters`、重複除去)のうち庭に存在するトークンを集め、2体以上いれば現在位置の平均点へ全員でグライドして1.5秒静止、その後「✨」反応を出して通常の徘徊へ戻る(`PetToken.GatherAsync`、徘徊ループは`_isGathering`フラグを見て自分から動くのを一時停止する)。「文字だからできる」体験として、単語が完成した瞬間に構成文字が実際に寄り添う。
...
| PetDetailView | PetDetailPresenter | Lv・経験値・満腹度・生産量・研究状況・研究速度バフ(グローバル+このキャラの応援)残り時間。「エサをあげる」「応援する（コスト表示）」「描き直す」ボタン(研究は完全自動のため選択・中止ボタンは無い)。`OnMoneyChanged`購読で応援ボタンの残高不足判定をリアルタイム更新。**「なでる」ボタンは2026-07-20に撤去**(庭でのタップ操作に統合。6.5節参照) |
...
(研究のカテゴリゲートは現在`ResearchSystem`に統合済み。5.10節参照。手動選択画面自体が2026-07-19に撤去されているため、旧`ResearchSelectPresenter`の記述はここから削除した。)

## 6.7 Toast(`UI/Views/Toast.cs`)

画面上部に表示され自動で消える通知。`Toast.Show(parent, message)`の静的呼び出しのみ、アニメーション(フェード等)は無く即座に表示・破棄。**2026-07-20**: 複数行メッセージに対応。表示時間は`\n`で数えた行数に応じて2.5秒(1行)〜最大6秒まで伸び、高さも1行あたり32px分自動で広がる(おかえりまとめ等、複数行の通知向け)。
